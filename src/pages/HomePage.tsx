...
import { ChevronDown, ChevronRight, Heart, PieChart, BarChart, ArrowRight, Building, Users, CheckCircle, Facebook, Twitter, Instagram, Linkedin, Menu, X, Wallet } from "lucide-react"
import { getVerifiedBeneficiaries } from '@/services/api'
...
const HomePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchBeneficiaries = async () => {
      try {
        const data = await getVerifiedBeneficiaries();
        setVerifiedBeneficiaries(data);
        setIsLoading(false);
      } catch (err) {
        setError('Failed to fetch verified beneficiaries. Please try again later.');
        setIsLoading(false);
      }
    };
        ...
        >
          <div className="flex flex-col md:flex-row items-center justify-between w-full">
            <div className="md:w-1/2 mb-8 md:mb-0">
              ...
              >
                <Button className="rounded-full px-6 py-4 md:px-8 md:py-6 text-base md:text-lg bg-gray-900 hover:bg-gray-800 text-white font-medium">
        ...
        >
          <div className="bg-[#7CE993] rounded-3xl p-8 md:p-16 text-center">
            ...
            <motion.p
                ...
                transition={{ duration: 0.5, delay: 0.4 }}
            >
              In collaboration with the Aptos Network, we're revolutionizing charitable giving. Whether it's through digital donations, smart volunteering, or decentralized fundraising, there are many ways to make a meaningful impact. Discover how you can contribute to our mission.
            </motion.p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {[
                { icon: Heart, title: "Direct impact", description: "Your donation goes directly to beneficiaries through secure technology, ensuring immediate and transparent support for our causes." },
                { icon: PieChart, title: "Verifiable track record", description: "Our projects' progress and outcomes are recorded transparently, providing an immutable history of impact." },
                { icon: BarChart, title: "Transparent operations", description: "We leverage innovative technology for transparent fund allocation, real-time reporting, and decentralized decision-making." }
              ].map((item, index) => (
                  <motion.div
                      key={index}
                      initial={{ opacity: 0, y: 20 }}
                    ...
                    >
                      <div className="p-6">
                        <div className="flex items-center mb-4">
                          <img src={beneficiary.logo} alt={beneficiary.name} className="w-16 h-16 rounded-full mr-4 object-cover" />
                          <div>
        ...
        >
          <div className="flex flex-col lg:flex-row justify-between items-start mb-12">
            <motion.div
                ...
                transition={{ duration: 0.5, delay: 0.2 }}
            >
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-medium mb-4">Got questions? we've got answers.</h2>
              ...
              <Button className="mt-6 rounded-full bg-gray-900 text-white hover:bg-gray-800 text-base md:text-lg font-medium">
